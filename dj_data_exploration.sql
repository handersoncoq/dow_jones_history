...
SELECT DATENAME(YEAR, date) AS year, stock,
	AVG("close") AS avgClosingPrice, MAX("close") AS maxClosingPrice
FROM dow_historic
...
(
	...
	RANK() OVER (PARTITION BY DATENAME(YEAR, date) ORDER BY AVG("close") DESC) AS rank
	FROM dow_historic
	GROUP BY DATENAME(YEAR, date), stock
) average
WHERE rank = 1
...
WITH cte_current AS
(
SELECT ROW_NUMBER() OVER (PARTITION BY stock ORDER BY date DESC) AS number, date, stock, "close", volume
FROM dow_historic
),
cte_previous AS
(
...
cte_volumePrice AS
(
SELECT T1.date, T1.stock, ABS(((T1."close"/T2."close")-1))*T1.volume AS volumePrice
...
INSERT INTO @table1
SELECT ROW_NUMBER() OVER (PARTITION BY stock ORDER BY date DESC) AS number, date, stock, "close"
FROM dow_historic;
...
cte_volumePrice AS
(
SELECT T1.date, T1.stock, LOG(T1."close"/T2."close") AS logChange
FROM cte_current T1, cte_previous T2
WHERE T1.number = T2.number AND T1.stock = T2.stock
)
